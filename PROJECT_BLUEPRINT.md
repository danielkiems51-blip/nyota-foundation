...
- **Payment Integration:** SmartPayPesa API (M-Pesa STK Push)
...
4. **API Orchestration:** `SmartPayPesaService` normalizes data, authenticates, and sends an STK Push request via SmartPayPesa.
5. **Callback Handling:** SmartPayPesa sends a POST notification to `/api/mpesa/callback/` to update transaction status.
...
├── nyota/              # Logic App
│   ├── services.py     # SmartPayPesa API Integration Logic
...
- **Security:** Environment variables for `SECRET_KEY` and SmartPayPesa credentials; `DEBUG=False` in production.
