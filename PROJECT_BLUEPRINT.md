...
- **Payment Integration:** Tuma API (M-Pesa STK Push)
...
4. **API Orchestration:** `TumaService` normalizes data, authenticates, and sends an STK Push request via Tuma.
5. **Callback Handling:** Tuma sends a POST notification to `/api/mpesa/callback/` to update transaction status.
...
├── nyota/              # Logic App
│   ├── services.py     # Tuma API Integration Logic
...
- **Security:** Environment variables for `SECRET_KEY` and Tuma credentials; `DEBUG=False` in production.
